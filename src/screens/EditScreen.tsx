...
export default function EditScreen({ route, navigation }: Props) {
  const initial = (route.params?.images || []).map((x, idx) => ({
    ...x,
    key: `${x.uri}-${idx}-${Date.now()}`,
  }));
  const [items, setItems] = useState(initial);
  const [exporting, setExporting] = useState(false);
  const [hd, setHd] = useState(false);
  const [optsVisible, setOptsVisible] = useState(false);
  const [pdfUi, setPdfUi] = useState<{
    paper: PaperPreset;
  ...
  const applyAdjustments = useCallback(
    async (key: string, adjustments: ImageAdjustments) => {
              ...
              <Pressable
                onPress={async () => {
                  try {
                    ...
                    setItems((prev) =>
                      prev.map((it) =>
                        it.key === item.key ? { ...it, uri: res.uri } : it
                      )
                    );
                  } catch {}
                }}
  ...
  const imagesForPdf: ImageInput[] = useMemo(
    ...
    try {
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: "Your PDF is ready to share.",
        },
        trigger: null,
      });
    ...
    <View className="flex-1 bg-background">
      <View className="px-6 pt-6 pb-6">
        <Animated.View entering={FadeInDown.duration(250)}>
          <Text
            className="text-3xl text-text-primary mb-3"
          ...
          <Text
            ...
            style={{ fontFamily: "Inter_400Regular" }}
          >
            Long-press and drag to reorder. Use the tools to edit, crop, or
            rotate each image.
          </Text>
        </Animated.View>
        ...
        <View className="flex-row mt-8 items-center space-x-4">
          <View className="flex-row items-center bg-surfaceElevated rounded-3xl px-5 py-4 shadow-medium border border-border-light flex-1">
            ...
            <Text
              className="text-text-secondary mx-3 text-base"
              style={{ fontFamily: "Inter_500Medium" }}
            >
              HD Quality
            </Text>
            {hd ? (
              ...
              <Pressable
                onPress={unlockHD}
                className="bg-accent-600 rounded-xl px-4 py-2 shadow-soft"
              >
                <Text
          ...
          >
            ...
            <Text
              className="text-text-primary ml-3 text-base"
              style={{ fontFamily: "Inter_500Medium" }}
            >
              Options
            </Text>
        ...
        onSelect={(key, ratio) => {
          ...
          setHandler(handlerId, (res: any) => {
            setItems((prev) =>
              prev.map((x) =>
                x.key === editorKey
                  ? { ...x, uri: res.uri, width: res.width, height: res.height }
                  : x
              )
            );
          });
